...
- ในฐานข้อมูลมีข้อมูลของหลักสูตร DSBA, IT, AIT, BIT โดยมี program_id คือ 'DSBA', 'IT', 'AIT', 'BIT'
- ตาราง program และ views (v_plan, v_semester_credits) มีคอลัมน์ program_id
- หากคำถามระบุชื่อหลักสูตร ให้กรองเงื่อนไข program_id เสมอ:
  - หลักสูตร IT: program_id = 'IT' (ระวัง: ใช้ = 'IT' เท่านั้น ห้ามใช้ LIKE '%IT%' เพราะจะไปตรงกับ BIT)
  - หลักสูตร BIT: program_id = 'BIT'
  - หลักสูตร DSBA: program_id = 'DSBA'
  - หลักสูตร AIT: program_id = 'AIT'
...
SQL: SELECT total_credits FROM program WHERE program_id = 'AIT' LIMIT 1
...
SQL: SELECT total_credits FROM program WHERE program_id = 'BIT' LIMIT 1

คำถาม: หลักสูตร IT ใช้เวลาเรียนกี่ปี
SQL: SELECT years FROM program WHERE program_id = 'IT' LIMIT 1
...
คำถาม: วิชา 06026200 เรียนชั้นปีที่เท่าไร
SQL: SELECT year FROM plan_item WHERE code = '06026200' LIMIT 1

คำถาม: วิชา 06026200 เรียนภาคการศึกษาใด
SQL: SELECT semester FROM plan_item WHERE code = '06026200' LIMIT 1
...
คำถาม: หลักสูตร BIT ปี 1 เทอม 2 เรียนกี่หน่วยกิต
SQL: SELECT credits FROM v_semester_credits WHERE program_id = 'BIT' AND year = 1 AND semester = 2 LIMIT 1
...
- ถามว่าเรียนชั้นปีที่เท่าไร ให้ SELECT year FROM plan_item WHERE code = ...
